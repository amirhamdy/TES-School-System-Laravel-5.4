                ...
                <div class="panel panel-default">
                    ...
                    <div class="panel-body">
                        ...
                        <form class="form-horizontal" role="form" method="POST"
                              action="{{ url('schedules/schedules/'.$schedule->id) }}">
                            {{ csrf_field() }}
                            {{ method_field('PATCH') }}
                            ...
                            <div class="form-group{{ $errors->has('start') ? ' has-error' : '' }}">
                                ...
                                <div class="col-md-6">
                                    <input id="start" type="datetime-local" class="form-control" name="start"
                                           value="{{ date('%Y-%m-%dT%H:%M:%S', strtotime($schedule->start)) }}" required
                                           autofocus>
                                    @if ($errors->has('start'))
                                        <span class="help-block">
                                ...
                                <div class="col-md-6">
                                    <select class="form-control m-b" name="exam_id">
                                        @foreach ($exams as $key => $exam)
                                            @if($schedule->exam_id == $key)
                                                <option selected value="{{$key}}">{{$exam}}</option>
                                            @else
